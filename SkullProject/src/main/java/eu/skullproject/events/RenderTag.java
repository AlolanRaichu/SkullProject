...
public class RenderTag {
    ...
    public void render(RenderPlayerEvent.Pre event) {
        String s;
        if(event.entityPlayer.getUniqueID().equals(CheckPlayer.UUID)) return;
        if(!event.entityPlayer.isSneaking() && (s = SkullProject.stringCache.get(event.entityPlayer.getUniqueID())) != null) {
            double offset = 0.3;
